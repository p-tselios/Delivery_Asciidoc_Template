...
:sectnumlevels: 6
:numbered:
:chapter-label: ch01_01_idm_configuration_with_ansible
:icons: font
...
<1> The default group for new users must already exists. This is defined here as an example.
...
<1> If the controller node is an IdM client, we can use it node for the configuration of IdM, otherwise we need to define another IdM client or server as the target node on which we will execute the playbook.
